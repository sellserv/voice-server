<script lang="ts">
  ...
  // Check if @everyone is active (any role overrides exist)
  ...
  async function loadData() {
    ...
    try {
      ...
      const [overrideData] = await Promise.all([
        api.get<ChannelPermissionOverride[]>(`/api/servers/${serverId}/channels/${channelId}/permissions`),
        fetchUsers(),
      ]);
  ...
  function selectTarget(type: 'role' | 'user', id: string) {
    ...
    if (id === EVERYONE_ID) {
      // For @everyone, show the common state across all role overrides
  ...
  async function saveOverride() {
    ...
    try {
      const serverId = getActiveServerId();
      if (isEveryone) {
        // Apply the same permissions to every role
        for (const role of $roles) {
          await api.put(`/api/servers/${serverId}/channels/${channelId}/permissions`, {
            target_type: 'role',
            target_id: role.id,
      ...
      overrides = await api.get<ChannelPermissionOverride[]>(
        `/api/servers/${serverId}/channels/${channelId}/permissions`,
      );
  ...
  async function deleteOverride() {
    ...
    try {
      const serverId = getActiveServerId();
      if (isEveryone) {
        // Remove all role overrides
        const roleOverrides = overrides.filter((o) => o.target_type === 'role');
        for (const o of roleOverrides) {
          await api.delete(`/api/servers/${serverId}/channels/${channelId}/permissions/role/${o.target_id}`);
        }
      } else {
        await api.delete(
          `/api/servers/${serverId}/channels/${channelId}/permissions/${selectedTarget.type}/${selectedTarget.id}`,
        );
      }
      overrides = await api.get<ChannelPermissionOverride[]>(
        `/api/servers/${serverId}/channels/${channelId}/permissions`,
  ...
  function addTarget(type: 'role' | 'user', id: string) {
    showAddMenu = false;
    // Pre-create empty override state on client side
  ...
  <div class="modal" onclick={(e) => e.stopPropagation()}>
    <div class="modal-header">
      <div class="header-info">
        <Icon name={isVoice ? 'volume' : 'hash'} size={20} class="header-icon" />
        <h3>#{channelName} Permissions</h3>
      </div>
        ...
        <div class="perms-panel">
          ...
          {:else}
            <div class="no-selection">
              ...
              <h3>Manage Channel Overrides</h3>
              <p>Select a role or user on the left to configure their specific permissions for this channel.</p>
            </div>
          {/if}
        </div>
      </div>
