...
// Only import PostgresAdapter when DATABASE_URL is set and pg is available.
// This avoids a top-level import error when the 'pg' package isn't installed.
...
if (DATABASE_URL) {
  try {
    ...
    // pg not installed — tests will be skipped
  }
}

describe.skipIf(!DATABASE_URL || !PostgresAdapter)('PostgresAdapter', () => {
  ...
  beforeEach(async () => {
    await adapter.exec('TRUNCATE test_users RESTART IDENTITY');
  });
