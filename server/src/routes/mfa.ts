...
export default async function mfaRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { totp_code: string } }>(
    ...
    async (request, reply) => {
      ...
      db.prepare("UPDATE users SET totp_enabled = 1, mfa_method = 'totp' WHERE id = ?").run(userId);
      logAuditEvent('mfa_enable', request.user.userId, null, request.ip);
  ...
  app.post<{ Body: { totp_code: string } }>(
    ...
    async (request, reply) => {
      ...
      logAuditEvent('mfa_disable', request.user.userId, null, request.ip);
